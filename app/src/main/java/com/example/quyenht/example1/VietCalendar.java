...
public class VietCalendar {
    ...
    public static String getLunarDate(int dd, int mm, int yy) {
        String[] can = new String[]{"Giáp", "Ất", "Bính", "Đinh", "Mậu", "Kỉ", "Canh", "Tân", "Nhâm", "Quí"};
        String[] chi = new String[]{"Tý", "Sửu", "Dần", "Mạo", "Thìn", "Tỵ", "Ngọ", "Mùi", "Thân", "Dậu", "Tuất", "Hợi"};
        long juliusDay = jdFromDate(dd, mm, yy);
        return can[(int) ((juliusDay + 9) % 10) - 1] + " " + chi[(int) ((juliusDay + 12) % 12)];
    }

    public static String getLunarMonth(int mLunarMonth,int mLunarYear ) {
        String[] can = new String[]{"Giáp", "Ất", "Bính", "Đinh", "Mậu", "Kỉ", "Canh", "Tân", "Nhâm", "Quí"};
        String[] chi = new String[]{"Dần", "Mạo", "Thìn", "Tỵ", "Ngọ", "Mùi", "Thân", "Dậu", "Tuất", "Hợi", "Tý", "Sửu"};

        int mod = (mLunarYear * 12 + mLunarMonth + 3) % 10;
        return can[mod] + " " + chi[mLunarMonth - 1];
    }

    public static String getLunarYear(int mLunarYear) {
    ...
    public static String[] getNameCanChi(String dateSolar){
        double Tz = 7.0;
        String[] separatedTime = dateSolar.split("-");
        int dateValue = Integer.parseInt(separatedTime[2].replaceFirst("^0*", ""));
        int monthValue = Integer.parseInt(separatedTime[1]);
        int yearValue = Integer.parseInt(separatedTime[0]);
        int[] dateLuna = getDateLuna(dateSolar);
        String namCanChi = getLunarYear(dateLuna[2]);
        String thangCanChi = getLunarMonth(dateLuna[1],dateLuna[2]);
        String ngayCanChi = getLunarDate(dateValue,monthValue,yearValue);
        String[] nameAmLich = {ngayCanChi,thangCanChi,namCanChi};
        return nameAmLich;
    }
